 ...
 * on device, `cd /system/etc; tar czvf /data/firmware.tgz firmware` (If you don't have tar on the board download it from http://busybox.net/downloads/binaries/latest/busybox-armv7l. Use: adb push busybox-armv7l /system/bin/busybox to put on the board. Then: chmod 0777 /system/bin/busybox. Either use 'busybox tar ...' or create a symbolic link named tar to busybox to use it)
